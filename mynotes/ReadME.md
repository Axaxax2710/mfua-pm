Мы обновили ваш README, заменив жёстко заданные имена `repoA` и `repoB` на более гибкие переменные `SOURCE_REPO` и `DEST_REPO` (для Bash) и параметры `-Source` / `-Destination` (для PowerShell). Теперь скрипты можно использовать для любых пар репозиториев, передавая нужные пути через аргументы или переменные окружения.
